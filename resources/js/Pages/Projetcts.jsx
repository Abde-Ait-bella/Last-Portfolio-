...
function Projects() {
    ...
    return (
        <div className="flex flex-col p-5 pb-10 items-center h-[70vh] overflow-auto 
            ...
            <div className="flex flex-col gap-6 w-full max-w-4xl">
                {data.map((project) => (
                    <motion.div
                        key={project.id}
                        className="bg-white/80 shadow-lg rounded-lg max-h-[11.5rem] overflow-hidden hover:shadow-xl transition-shadow duration-300 flex"
                        whileHover={{
                    ...
                    >
                        ...
                        <div className="w-1/3 bg-gray-300/50 overflow-hidden relative">
                            <motion.img
                                src={project.image || "./images/projectsScreenShots"}
                                alt={project.name}
                                className="w-full max-h-[11.5rem] object-cover"
                        ...
                        <div className="w-2/3 p-6 flex flex-col">
                            <div className="flex justify-between items-center mb-3">
                                <h2 className="text-xl font-bold capitalize text-gray-800">{project.name}</h2>
                                <div className="flex space-x-3">
                                    <a href={project.website || "#"} target="_blank" rel="noopener noreferrer" className="text-gray-700 hover:text-blue-600 flex items-center">
                                        <span className="mr-1 text-sm overflow-hidden max-w-30 text-ellipsis whitespace-normal">{project.domain || "Live"}</span>
                                    ...
                                    <a href={project.github || "#"} target="_blank" rel="noopener noreferrer" className="text-gray-700 hover:text-black flex items-center">
                                        <span className="mr-1 text-sm">Code</span>
                            ...
                            <p className="text-gray-600 mb-4 h-[3rem] overflow-hidden text-ellipsis whitespace-normal line-clamp-2">{project.description}</p>

                            <div className="mt-2">
                                <div className="flex flex-wrap gap-2">
                                    {project.technologies ? project.technologies.map((tech, index) => (
                                        <span key={index} className="px-2 py-1 bg-gray-200/70 text-gray-800 text-xs rounded-md">
                                            {tech}
                                        </span>
                                    )) : (
